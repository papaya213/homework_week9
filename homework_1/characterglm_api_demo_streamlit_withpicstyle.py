"""
一个简单的demo，调用CharacterGLM实现角色扮演，调用CogView生成图片，调用ChatGLM生成CogView所需的prompt。请提前通过环境变量配置ChatGLM API
...
def draw_new_image():
    ...
    image_prompt = selected_option + image_prompt.strip()
...
with st.container():
    ...
    pic_style_options = ["二次元风格", "黑白线条风格", "写实风格"]
    selected_option = st.selectbox("生成图片风格:", pic_style_options)
    st.write(f"生成图片风格： {selected_option}")
...
def start_chat():
    ...
    if not query:
        ...
        if not response_stream:
            ...
            st.session_state["history"].append(TextMsg({"role": "assistant", "content": response_stream}))
